  ...
  <div class="bookLists">
    ...
    <div class="load-more" ref="load" :class="[mins.length>0?'padding300':'padding200']">
      <mt-loadmore class="loadmore" ref="loadmore" :top-method="loadTop" :bottom-method="loadBottom" :auto-fill="false"
        ...
        <div class="book-list">
          <book-list :book="book" v-for="(book,index) in bookLists" :key="index" :cate="cate" :isLoadMore="isLoadMore"></book-list>
  ...
  export default{
    ...
    methods: {
      ...
      _getBookLists(gender, type, major, minor = '', start = 0, limit = 20){
        api.get(`/category-info?gender=${gender}&type=${type}&major=${major}&minor=${minor}&start=${start}&limit=${limit}`, {}, (res) => {
          this.bookLists = this._unEscape(this.normalizeBooks(res.books))
        })
      ...
      loadBottom(){
        ...
        Indicator.open('加载中');
        api.get(`/category-info?gender=${this.gender}&type=${this.type}&major=${this.major}&minor=${this.minor}&start=${this.currentPage * 20}&limit=20`, {}, (res) => {
          this.bookLists = this.bookLists.concat(this._unEscape(this.normalizeBooks(res.books)));
          setTimeout(() => {
            Indicator.close();
