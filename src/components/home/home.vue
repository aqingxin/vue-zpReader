...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    methods: {
      getBookBill(){
        Indicator.open('加载中');
        api.get('/rank/54d42d92321052167dfb75e3', {}, (res) => {
          if (res.ok) {
            this.maleList = this._unEscape(this.normalizeBooks(res.ranking.books.slice(0, 5)));
            Indicator.close()
          }
        });
        api.get('/rank/54d43437d47d13ff21cad58b', {}, (res) => {
          if (res.ok) {
            this.femaleList = this._unEscape(this.normalizeBooks(res.ranking.books.slice(0, 5)))
          }
        })
